...
public class MazeBrickWall
{
    ...
    private void setExtrution(){
        for(int i = 0; i < hSplit; i++){
            for(int j = 0; j < vSplit; j++){
                extrude[i][j] = 0.06f + (float)Math.random() * (0.14f - 0.06f);
            }
        }
    }
        ...
        for (int h = 0; h < hSplit; h++) {
            for (int v = 0; v < vSplit; v++) {
                ...
                if (h % 2 == 0) {
                    ...
                    mm.addTranslationBaseCoords((float) i * cellSize, height / hSplit * h, (float) j * cellSize + (cellSize / vSplit * v));
                } else {
                    mm.addScale(width + extrude[h][v], height / hSplit - fua, cellSize / vSplit - fua);
                    mm.addTranslationBaseCoords((float) i * cellSize, height / hSplit * h, (float) j * cellSize + (cellSize / vSplit * v) + cellSize / vSplit / 2);
                }
                mm.setShaderMatrix();
